...
public class EventArea 
{
	...
	////"服装提案開始"ボタンが押されたら、W5地域入力画面作成からW6服装提案画面に遷移
		...
		bt.setOnAction((ActionEvent) ->
			{ 
				try {
					...
					List<Clothes> result = new DataClothing().getClothes(areaname); //地域名称から天気情報を取得し提案された服装を格納
					new DataClothing().orderList(result); //データリスト取得
				}catch(Exception e) {
	...
	//-------------------------------------------- 
	//void transitionResult(Button bt, ListView<String> lv) 
	////"キャンセル"ボタンが押されたら、W5地域入力画面作成からW4ホーム画面に遷移
	//bt:"服装提案"ボタン
	//--------------------------------------------
